...
export function CoreTipCard({ lang, onOpen }: { lang: Lang; onOpen?: () => void }) {
  return (
    <div
      onClick={onOpen}
      style={s(
        'background:#221C15;color:#F0EADC;border-radius:18px;padding:20px;position:relative;overflow:hidden' +
          (onOpen ? ';cursor:pointer' : ''),
      )}
    >
      <div style={s('position:absolute;right:-50px;top:-50px;width:170px;height:170px;border-radius:50%;background:radial-gradient(circle,#3C6B58 0%,transparent 70%);opacity:.55')} />
...
function StoryCard({ story, lang }: { story: Story; lang: Lang }) {
  ...
  return (
    <div
      ...
      style={s('cursor:pointer;background:#FFFFFF;border:1px solid #ECE6DA;border-radius:16px;padding:15px 16px')}
    >
